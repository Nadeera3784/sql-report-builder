  ...
  }})
{
  ...
  return (
    <div>
      {' From '}
      <input
        className={classNames("rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50")}
...
export function IndexCell({value, row, column}) {
  return(
...
export function DateCell({ value }) {
  const formated_date  =   moment(value).format('Y-M-DD');
   return(
      <span className={classNames("text-sm text-gray-500")}>
          {formated_date}
      </span>
...
export function ScheduleDateCell({ value }) {
    const formated_date  =   moment(value).format('Y-M-DD hh:mm:ss A');
     return(
        <span className={classNames("text-sm text-gray-500")}>
            {formated_date}
        </span>
...
function Table({ columns, data , setSelection, selection, onClickBulkDelete}) {
  const {
    getTableProps,
    getTableBodyProps,
    headerGroups,
  ...
  return (
    <>
      <div className="sm:flex sm:gap-x-2">
        <DeleteActionButton selection={selection} onClickBulkButtonDelete={onClickBulkDelete}/>
        <GlobalFilter
